...
run_gat_mlp.py
==============
GAT + MLP baseline: graph attention network with standard MLP head.
This is the direct comparison to KAN-GAT — same GAT encoder,
different classifier head (MLP vs KAN).

Usage:
    python run_gat_mlp.py
"""
...
from models import GATMLP, GATLinear, count_parameters
...
def train_gat(model_class, model_name):
    """Train and evaluate a GAT-based model with 5-fold CV."""
    print("=" * 60)
    print(f"{model_name}")
    print("=" * 60)
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(node_indices, labels)):
        ...
        model = model_class(in_dim=in_dim, hidden_dim=HIDDEN_DIM)
    ...
    results_path = os.path.join(RESULTS_DIR, f"results_{safe_name}.json")
...
def run_all():
    print("Training GAT+MLP (standard)...")
    gat_mlp_results = train_gat(GATMLP, "GAT+MLP")

    print("\nTraining GAT+Linear (ablation)...")
    gat_linear_results = train_gat(GATLinear, "GAT+Linear")

    return {"GAT+MLP": gat_mlp_results, "GAT+Linear": gat_linear_results}
